...
`docker compose -f docker-compose.selfhost.yml up -d --build` démarre **3 conteneurs** : `postgres` + `api` (build depuis `src/EbiosRM.Api/Dockerfile`) + `web` (nouveau `frontend/Dockerfile` : build Vite multi-stage -> nginx). **Fichier compose séparé** (`docker-compose.selfhost.yml`, standalone avec son propre volume `pgdata`) plutôt qu'un profil dans `docker-compose.yml` : Compose interpole *tout* le fichier même pour `docker compose up` sans profil, donc un `${JWT_SECRET:?...}` requis sur le service `api` cassait le `docker compose up` du workflow de dev. `docker-compose.yml` reste inchangé (Postgres seul).
...
### Import JSON (point 2, 2ᵉ moitié)
- **`RecableurClesEtude`** (extrait de la duplication) : moteur commun de ré-attribution des clés. `EnregistrerEtude` → `ReserverId` sur chaque racine → `Rattacher(entite, ...fk)`. Mode `strict` : une référence introuvable dans la table de correspondance lève `ReferenceIntrouvableException` (import) ; laissée telle quelle (duplication). **Piège corrigé** : ne pas remapper les FK *shadow* vers le propriétaire d'une entité owned (`MesureEcosysteme.PartiePrenanteId` shadow) — seulement les vraies propriétés CLR (`EvenementIntermediaire.PartiePrenanteId`, `ActionElementaire.BienSupportId`).
- **`ServiceImportEtude`** : désérialise le fichier en entités du domaine via `OptionsEntitesInternes` — un `DefaultJsonTypeInfoResolver` modifier qui, **pour les seuls types sous `...CoreEngine.Domain`**, ouvre `CreateObject` (ctor privé), les setters privés, et les champs de backing privés (`_referentiels`, `_mesures`, `_scenariosDeRisqueIds`...). Puis même re-câblage que la duplication, en mode strict, sous transaction. Tout échec → message + 400, jamais 500, rien d'écrit.
- **`POST /api/v1/etudes/importer`** : le corps de la requête EST le fichier `.json`. Route sans `etudeId` → le middleware d'isolation ne s'applique pas. Garde-fou taille 15 Mo. L'appelant devient Proprietaire. `formatVersion != 1` → message clair.
...
### Étape 2 — Modes opératoires / chemins d'attaque (commit 2)
- **`ModeOperatoireBibliotheque`** (`IEntreeBibliotheque`) : Nom, Description?, ProbabiliteSuccesTypique?/DifficulteTechniqueTypique? (1..4), + owned `ActionElementaireBibliotheque` (Ordre, Description, `PhaseActionElementaire`, **CibleBienSupport** = libellé texte, pas un Id d'étude, TechniqueMitre?). `ActionElementaireBiblioEntree` record struct pour (re)construire. `Creer(...)` + `Systeme(cle, …, params entrées)`.
- **`CatalogueSysteme.ModesOperatoires`** : 5 modes types (rançongiciel par hameçonnage, accès distant exposé, rebond prestataire, vuln web → BDD, domination AD) avec 4-6 actions chacun + techniques MITRE.
- **EF** : `OwnsMany(Actions)` → tables `bibliotheque_modes_operatoires` / `bibliotheque_actions_elementaires` (FK shadow `ModeOperatoireBibliothequeId`, cascade). Owned = auto-inclus, le dépôt générique n'a pas besoin de `.Include()`. Migration `20260829150432_AjoutModeOperatoireBibliotheque`. Seed régénéré (8 tables biblio).
- **`Program.cs`** : `GET/POST/DELETE /api/v1/bibliotheque/modes-operatoires` (recherche `?q=` jusque dans les actions/MITRE). Records `AjouterModeOperatoireBiblioRequest` + `ActionElementaireBiblioRequest`.
- **Frontend** : 7ᵉ onglet `OngletModesOperatoires` (dépliable — liste ordonnée des actions phase/cible/MITRE, formulaire d'ajout avec éditeur d'actions inline). `AtelierPage.AjoutModeOperatoire` (A4) : `DepuisBiblio<ModeOperatoireBiblio>` pré-remplit description + cotations + lignes d'actions (bien support = premier de l'étude par défaut, libellé de cible reporté entre parenthèses dans la description + avertissement « associez chaque action au bon bien support »).
- **Tests** : `BibliothequeTests` +1 (catalogue avec actions ordonnées + phases + MITRE, recherche par technique, ajout perso, refus si aucune action, catalogue système non supprimable). 265 backend, 25 frontend. Playwright : 7ᵉ onglet OK (catalogue + dépliage des actions).

### Étape 3 — Bibliothèque partagée communautaire (commit 3)
- **Table à part**, aucune modif des 7 tables d'entrées : `PublicationBibliotheque` (Id, TypeEntite slug, EntiteId, ProprietaireId, PublieLeUtc, Masquee) + owned `SignalementBibliotheque` (SignalePar, Motif?, unique par (publication, signaleur)). `Signaler()` ignore l'auteur et les doublons ; `Masquee`=true dès `SeuilMasquage=3` signaleurs distincts.
- **`IEntreeBibliotheque.CopiePrivee(Guid)`** ajouté aux 7 entités (délègue à leur `Creer`) — sert à l'import. `IBibliothequeRepository.ListerParIdsAsync<T>` ajouté.
- **`ServiceBibliothequeCommunautaire`** (`Modules/Bibliotheque/`, injecté scoped) : `Publier` / `Retirer` / `Importer` (clone + `_db.Add`) / `Signaler` / `Lister` (charge les entités par ids, joint le `NomAffiche` de l'auteur, filtre `q` via `JsonSerializer.Serialize`) / `IdsPubliesAsync`. Dispatch `slug string → T` par `switch` + helpers `Cast<T>` / `ChargerAsync` / `ChargerPlusieursAsync`. `Types` = 7 slugs (`mesure`, `source-risque`, `partie-prenante`, `valeur-metier`, `bien-support`, `evenement-redoute`, `mode-operatoire`).
- **`Program.cs`** : `GET /bibliotheque/communaute/{type}`, `GET /bibliotheque/mes-publications`, `POST|DELETE /bibliotheque/communaute/{type}/{id}/publier`, `POST .../importer`, `POST .../signaler`. `VueEntreeBiblio(object)` : `switch` sur le type runtime → la bonne `Vue*Biblio` locale. Record `SignalerBiblioRequest`.
- **EF** : tables `bibliotheque_publications` / `bibliotheque_signalements`. Migration `20260829151628_AjoutBibliothequeCommunautaire`. Seed régénéré (10 tables biblio).
- **Frontend** : bascule « Ma bibliothèque / Communauté » (segmented control) au-dessus des onglets. `VueCommunaute` (par type, `texteEntree(onglet, entree)` pour le rendu générique, boutons Importer + Signaler avec `window.prompt` motif). `Liste` gagne `slug?` → bouton « Publier » / « Publié ✓ » sur les entrées perso (charge `mesPublicationsBiblio`). `api.ts` : `EntreeCommunaute` + 6 fonctions.
