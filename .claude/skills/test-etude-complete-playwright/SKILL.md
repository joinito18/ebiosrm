...
- **Authentification (depuis le chantier auth)** : `appsettings.Development.json` doit contenir `"Jwt": { "Secret": "..." }` (n'importe quelle chaine suffisamment longue en dev), sinon le backend refuse de demarrer. Tous les endpoints exigent desormais un jeton -- le script gabarit s'inscrit automatiquement avec un compte jetable en debut d'execution (voir plus bas), pas besoin de le faire a la main.
...
- `DELETE /api/v1/etudes/{id}` existe maintenant (purge complete, voir PROJECT_CONTEXT.md) -- pour nettoyer une etude de test creee pendant une session, l'appeler directement (avec le jeton du compte qui l'a creee, ou n'importe quel compte connecte -- toutes les etudes sont visibles par tous) plutot que de `TRUNCATE` les tables a la main.
- Les liens de telechargement de rapport PDF sont des boutons (`BoutonTelechargerRapport`), pas des `<a href>` -- ils declenchent un telechargement via blob JS, pas une navigation. Avec Playwright, capter l'evenement `download` (`page.waitForEvent('download')`) plutot que de suivre un `href`.
